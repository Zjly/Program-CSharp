...
namespace ClassSchedule {
	public class VerificationCodeRecognition {
		internal static void GetTemplate() {
		...
		private static void DivideToImage(byte[,] matrix, int color, int count, char letter) {
			...
			byte[,] newMatrix = new byte[16, 16]; // 分隔后字符的矩阵
			SaveToMatrix(resultImage as Bitmap, newMatrix);

			string path = @"../../image/template/" + letter.ToString().ToUpper() + ".txt";
			string s = MatrixToString(newMatrix); // 该字母的特征字符串
			// 向文件中追加信息
